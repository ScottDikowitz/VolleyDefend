(function () {
  ...
  Base.prototype.changeColor = function(color) {
    ...
    this.color = color;
    var that = this;
    setTimeout(function(){that.color = prevColor;}, 800);
  };
  ...
  Base.prototype.fireMissile = function(ctx) {
    var mouse = this.game.mouse;
    this.nosePos(ctx);
    this.game.friendlyMissiles.push(new VolleyDefend.Interceptor({pos: [this.end_x, this.end_y], game: this.game, vel: [mouse.x - this.pos[0],  mouse.y - this.pos[1]], mouse: mouse}));
  };
